...
#endif
#ifdef eon
    return ((number = execl(shell, shell, "-c", string, 0)) == -1) ? -1 :	/* couldn't start the shell */
...
char *string;
{
    ...
    for (i = 0, envp = environ; *envp; envp++)
	i += strlen(*envp) + 1;
...
/* DEBUG and leap year fixes thanks to Mark Alexander <uunet!amdahl!drivax!alexande>*/
#ifdef DEBUG
    printf
	("mstonix:  year=%d  month=%d  day=%d  hour=%d  min=%d  sec=%d\n",
    ...
    if (year % 4 == 0 && year % 400 != 0 && month >= 3)	/* if this is a leap year and month */
...
struct name *np;
{
	...
	Fclose(fd);
	np->n_time =
	    mstonix((unsigned int) fm.date, (unsigned int) fm.time);
	np->n_flag |= N_EXISTS;
    }
#endif
#ifdef eon
